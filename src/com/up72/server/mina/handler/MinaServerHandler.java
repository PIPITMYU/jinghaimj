...
//    public static ExecutorService workService = Executors.newFixedThreadPool(200);
    ...
    static class ClosedTask implements Runnable {
        ...
        public void run() {
            try{
                ...
                Player currentPlayer = TCPGameFunctions.getPlayerFromSession(session);
                if (currentPlayer!=null&&Cnst.PLAYER_STATE_DATING.equals(currentPlayer.getPlayStatus())&&currentPlayer.getSessionId().equals(session.getId())) {
                	TCPGameFunctions.playerMap.remove(currentPlayer.getUserId());
